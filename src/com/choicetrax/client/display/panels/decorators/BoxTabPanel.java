 ...
 * </p>
 */
public class BoxTabPanel extends Composite
		implements
		TabListener,
		SourcesTabEvents,
		HasWidgets,
		...
		IndexedPanel
{
	...
	private class UnmodifiableTabBar extends TabBar
	{
		...
		protected SimplePanel createTabTextWrapper() {
			return BoxTabPanel.this.createTabTextWrapper();
		}
	}

	private UnmodifiableTabBar		tabBar	= new UnmodifiableTabBar();
	private TabbedDeckPanel			deck	= new TabbedDeckPanel( tabBar );
	private BoxDecoratorPanel		boxPanel = null;
	private TabListenerCollection	tabListeners;
	
	private ArrayList<ClickableImageOnOff> clickableTabImages = new ArrayList<ClickableImageOnOff>();
	...
	public void setHeight( String height ) {
		this.boxPanel.setHeight( height );
	}
	...
	public void add( Widget w, ClickableImageOnOff clickableTabImage ) 
	{
		int widgetIndex = getWidgetCount();
		insert( w, clickableTabImage, widgetIndex );
		
		clickableTabImages.add( widgetIndex, clickableTabImage );
	}
	...
	private void setTabImages( int tabIndex ) 
	{
		// custom addition to turn off non-selected clickable images
		Iterator<ClickableImageOnOff> i = clickableTabImages.iterator();
		for ( int x=0; i.hasNext(); x++ ) 
		{
			ClickableImageOnOff clickableImage = i.next();
			
			if ( x == tabIndex ) 
				clickableImage.setStateOn();
			else 
				clickableImage.setStateOff();
		}
	}
